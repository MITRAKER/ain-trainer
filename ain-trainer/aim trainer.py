...
def end_screen(win, elapsed_time, target_pressed, clicks):
    ...
    accurecy = round((target_pressed / clicks) * 100, 1)
    accurecy_label = LABEL_FONT.render(f"Accurecy: {accurecy}%", 1, "white")

    win.blit(time_label, (get_middle(time_label), 100))
    win.blit(speed_label, (get_middle(speed_label), 200))
    win.blit(hits_label, (get_middle(hits_label), 300))
    win.blit(accurecy_label, (get_middle(accurecy_label), 400))
...
def main():
    ...
    pygame.time.set_timer(TARGET_EVENT, TARGET_INCREMENT) # 400ms = 1 second
